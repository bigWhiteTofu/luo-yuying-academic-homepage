...
http.createServer(async (request, response) => {
  ...
  if (url.pathname === "/api/visit" && request.method === "POST") { const data = await body(request); visits.push({ ip: "127.0.0.1", country: "CN", region: "Shanghai", city: "Shanghai", path: data.path || "/", referrer: data.referrer || "", visited_at: new Date().toISOString() }); return send(response, 201, { ok: true }); }
  if (url.pathname === "/api/message" && request.method === "POST") { const data = await body(request); messages.unshift({ id: messages.length + 1, ip: "127.0.0.1", country: "CN", region: "Shanghai", city: "Shanghai", message: data.message, created_at: new Date().toISOString() }); return send(response, 201, { ok: true }); }
